...
mainController.loginPost = function (req, res, next) {
  ...
  if (postData.username && postData.password) {
    request({
      url: 'http://129.213.124.20:3000/cronos/api/v1/login',
      method: 'POST',
      headers: {
        'Authorization': 'Basic MTAx'
      },
      json: true,
      body: postData,
...
let handleMWLogin = function (error, response, body, req, res, next, postData) {
  if (error) {
    return next(error);
  } else {
    if (response.statusCode !== 200) {
      winston.error('Error logging in with the MW');
      return res.redirect('/');
    } else if (body.user.roles.some(role => role === 'admin')) {
      const user = body.user;

      userSchema.getUserByUsername(user.id, function (error, persisteduser) {
        if (error) {
          return next(error);
        } else {
          if (!persisteduser) {
